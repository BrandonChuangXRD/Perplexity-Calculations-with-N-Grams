...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="choose between unigram(1), bigram(2), and trigram(3)",
    )
    parser.add_argument(
        "--train", "-t", default=BILLION_TRAIN_FILE, help="choose training token file"
    )
    parser.add_argument(
        "--predict",
        "-p",
        default=BILLION_DEV_FILE,
        help="choose token file to predict on",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="find perplexities of sanity, train, and dev tokens (excludes test)",
    )
    parser.add_argument(
        "--test", action="store_true", help="find perplexities of test tokens"
    )
    parser.add_argument(
        "--linear",
        action="store_true",
        help="toggles linear interpolation (superseeds add-1 smoothing)",
    )
    parser.add_argument(
        "--lambdas",
        nargs=3,
        type=float,
        default=[0.1, 0.3, 0.6],
        help="for linear interpolation: three floats separated by spaces (in order)",
    )
    parser.add_argument(
        "--smooth", "-s", action="store_true", help="enable smoothing for the MLE"
    )
    parser.add_argument("--sanity", action="store_true", help='test case for "HDTV ."')
    parser.add_argument(
        "--alpha", "-a", default=0, type=int, help="Choose value for add smoothing"
    )
